...
vector<vector<string> > front_end() {
	...
	cout<<"Please enter schema parameters: \n 1.mark target column with x\n 2.if not specified, use _\n 3.# par need to match: e.g. a,b,NA,d,NA,f[ENTER]\n"<<endl;
...
int main()
{
	...
	vector<vector<vector<string>>> local_conj_queries = unfold_to_local_schema(global_conj_queries);

	// print midend
    print_mid_end_vector(local_conj_queries);

	string sql = local_query_to_sql(local_conj_queries);
	cout<<sql<<endl;

	// give sql to backend
	//print_result(sql);

	setup_db_connection("example.db");
	query_db(sql);
	close_db_connection();
